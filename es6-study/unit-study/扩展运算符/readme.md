...
在以往，我们给函数传不确定参数数量时，是通过arguments来获取的,看下代码
```
function sum() {
...
得到的args是一个数组，直接对数组进行操作会比对伪数组进行操作更加方便，还有一些注意点需要注意: **扩展运算符只能放在最后一个参数**
```
...
console.log(person) // { name: 'ddd', age: 28 }
...
console.log(swrcopy) // { name: '铲屎官', age: 18, pets: [ '丸子' ] }
// 此时我们新增swr的属性
swr.pets.push('money')
console.log(swr) // { name: '铲屎官', age: 18, pets: [ '丸子', 'money'] } }
// 但是swrcopy却不会受swr影响
console.log(swrcopy) // { name: '铲屎官', age: 18, pets: [ '丸子' ] }
